...
export async function notifySubscribers(post: {
      ...
      return transporter.sendMail({
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0f1e; color: #e5e7eb; border-radius: 12px; overflow: hidden;">
            ${post.image ? `<img src="${escapeHtml(post.image)}" alt="" style="width: 100%; height: 200px; object-fit: cover;" />` : ""}
            <div style="padding: 24px 30px 30px;">
              <p style="font-size: 13px; color: #60a5fa; margin: 0 0 8px; text-transform: uppercase; letter-spacing: 1px;">New Blog Post</p>
              <h1 style="color: white; font-size: 22px; margin: 0 0 12px;">${escapeHtml(post.title)}</h1>
              <p style="color: #9ca3af; line-height: 1.6; margin: 0 0 24px;">${escapeHtml(post.description)}</p>
  ...
  results.forEach((r, i) => {
    if (r.status === "rejected") {
      console.error(`Subscriber email failed [${subscribers[i].email}]:`, r.reason?.message || r.reason);
    }
  });
